...
class PKBSohoScoring(BaseService, DataLoader):
    """ПКБ SOHO Scoring"""
    instance: Lead
    ...
    def run(self) -> Optional[dict]:
        logger.info("запрос на получение soho балл для заемщика iin %s", self.instance.borrower.iin)
        response = self.fetch(
    ...
    def handle_400(self, response: Response):
        logger.error("PKBSohoScoring.handle_400: response error %s", response.text)
    ...
    def prepared_data(self, data) -> dict:
        self.get_full_name_from_response(data)
    ...
    def get_full_name_from_response(self, data: dict):
        """
        SohoScoring возвращает ФИО по иин, сохраним в бд
        """
        if isinstance(data, dict):
            self.instance.last_name = data.get('last_name')
            self.instance.first_name = data.get('first_name')
            self.instance.middle_name = data.get('middle_name')
            self.instance.save(update_fields=['last_name', 'first_name', 'middle_name'])
