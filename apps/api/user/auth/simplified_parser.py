...
class BankStatementValidationError(Exception):
    """Custom exception for bank statement validation errors."""
    pass
...
class KaspiBankStatementParser:
    ...
    def validate_statement(self):
        """
        Validates that the content is actually a Kaspi Bank statement.

        Raises:
            BankStatementValidationError: If the content doesn't appear to be a valid statement
        """
        # Markers that should be present in a Kaspi Bank statement
        markers = [
            'Выписка по счету',
            'Kaspi Gold',
            'Kaspi Bank',
        ...
        # Check for the presence of at least 2 markers
        matches = sum(1 for marker in markers if marker.lower() in self.content.lower())
        if matches < 2:
            raise BankStatementValidationError("Документ не является выпиской Kaspi Bank")

        return True

    def extract_card_number(self):
        """Extract the last 4 digits of the card number from the statement."""
        # Pattern to match card numbers like "Карта *1234" or "Card *1234"
        patterns = [
            r'Карта\s+\*+(\d{4})',
            r'Card\s+\*+(\d{4})',
            r'Kaspi\s+Gold\s+\*+(\d{4})',
            r'счет\s+\*+(\d{4})',
            r'счёт\s+\*+(\d{4})',
            r'account\s+\*+(\d{4})'
        ]
    ...
    def parse(self):
        ...
        """
        # First validate this is actually a Kaspi Bank statement
        self.validate_statement()

        # Extract card number
        self.card_number = self.extract_card_number()

        # Determine the period of the statement
